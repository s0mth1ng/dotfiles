...
local git = {
  ...
  stage = function()
    vim.fn.VSCodeNotifyRange("git.stageSelectedRanges", vim.fn.line("v"), vim.fn.line("."), 1)
  end,
  revert = function()
    vim.fn.VSCodeNotifyRange("git.revertSelectedRanges", vim.fn.line("v"), vim.fn.line("."), 1)
  end,
  ...
  diff_line = function()
    vim.fn.VSCodeNotify("gitlens.diffLineWithWorking")
  end,
}
...
vim.keymap.set({ 'v' }, "<leader>s", git.stage)
vim.keymap.set({ 'v' }, "<leader>r", git.revert)
vim.keymap.set({ 'v' }, "<leader>gd", git.diff_line)
